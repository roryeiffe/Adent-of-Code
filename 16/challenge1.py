...
class range_set():
	def __init__(self,range1,range2):
		...
		self.max2 = range2[1]
	def __str__(self):
		return "Values must be between " + str(self.min1) + " and " + str(self.max1) + " or between " + str(self.min2) + " and " + str(self.max2)
...
for i in range(len(L)):
	...
	if line == "your ticket:":
		...
		r = range_set(parse_range(split1[0]),parse_range(split1[1]))
		ranges.append(r)
